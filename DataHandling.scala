...
val best12DaysDF: DataFrame = (retailerDataFrame
    .withColumn("TotalSales", col("QUANTITYORDERED") * col("PRICEEACH"))
    .groupBy("ORDERDATE")
    .agg(sum("TotalSales").alias("total_sales"))
...
val retailerDataFramewithsales: DataFrame = retailerDataFrame.withColumn("TotalSales", col("QUANTITYORDERED") * col("PRICEEACH"))

val productDF: DataFrame = retailerDataFramewithsales
  .withColumn("TotalSales", when((col("YEAR_ID") === 2003) && (col("MONTH_ID") === 1), col("TotalSales") * 2)
  .when((col("YEAR_ID") === 2003), col("TotalSales")))
  .groupBy("PRODUCTCODE")
...
val sales2017: Sales = salesDS.filter(sale => sale.year == 2017).head()

println(s"Sales for 2017 is ${sales2017.euros}")

val maximumSales: Sales = salesDS.orderBy(desc("euros")).limit(1).head()
...
val multiSales2016: Sales = multiSalesDS
                            .filter($"year" === 2016)
                            .groupBy("year")
                            .agg(sum("euros").cast("int").as("euros"))
                            .as[Sales]
                            .first()
...
val maximumMultiSales: Sales = multiSalesDS
                             .groupBy($"year")
                             .agg(sum("euros").cast("int").as("euros")) // Explicitly cast to int
                             .orderBy(desc("euros"))
                              .limit(1)
                             .as[Sales]
                              .first()
...
val my_ex3_folder: String = "streamingFolder"
...
val bestDaysDFStreaming: DataFrame = (retailerStreamingDF
...
// MAGIC My output was similar to the static data task.
// MAGIC
// MAGIC If you had problems, what were they?
// MAGIC And what do you think were the causes for those problems?
// MAGIC
// MAGIC I have answered the questions in the cell below.
// MAGIC

// COMMAND ----------

The streaming example works perfectly for me, In the begining I misunderstood the task that I have to create a file and provide a complete path to that file, but when I understood the flow of the code it makes my task easier.
The final out displayed first 6 rows having the top sales.
I got the same output as the earlier static data task, it contained 6 rows for the top sales.
My output was similar to the static data task.
I got the same output as the earlier task, but some difficulties which I faced is understanding the flow, and creating a dataframe having similar structure. I was getting null output but later in reading the data from the csv file I added the delimiter ';' which solved my problem.
